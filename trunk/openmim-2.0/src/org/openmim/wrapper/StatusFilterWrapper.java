...
*/
public class StatusFilterWrapper
implements MessagingNetwork
{
  private final static Logger CAT = Logger.getLogger(StatusFilterWrapper.class.getName());
...
*/
  ...
  private void removeMessagingNetworkListener0(MessagingNetworkListener l, boolean reportNonExistingLs)  throws MessagingNetworkException
  {
    ...
    if (myL == null)
    {
      if (reportNonExistingLs)
        if (Defines.DEBUG && CAT.isDebugEnabled()) CAT.debug("IMListener "+l+" was not registered, removeMNL() request ignored.");
      return;
    }
  ...
  { wrapped.sendMessage(srcLoginId, dstLoginId, text); }
  */
  ...
  */
  ...
  { return wrapped.searchUsers(srcLoginId, emailSearchPattern,
      nickSearchPattern, firstNameSearchPattern, lastNameSearchPattern);
  }

  public long startLogin(String srcLoginId, String password, String[] contactList, int status) 
  throws MessagingNetworkException
  {
    return wrapped.startLogin(srcLoginId, password, contactList, status);
  }

  public long startSendMessage(String srcLoginId, String dstLoginId, String text) 
  throws MessagingNetworkException
  {
