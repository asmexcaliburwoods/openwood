  ...
  try
  {
    ...
    if (c.equals(boolean.class))
    {
      ...
      if ("true".equalsIgnoreCase(value) || "yes".equalsIgnoreCase(value))
        f.set(assignTo, new Boolean(true));
      else
        if ("false".equalsIgnoreCase(value) || "no".equalsIgnoreCase(value))
          f.set(assignTo, new Boolean(false));
        else
          throw new AssertException("invalid value: "+StringUtil.quote(value)+" for boolean field " + f.getName() + " of class " + c.getName());
    }
    else
    {
      Constructor con;
