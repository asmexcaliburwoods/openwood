...
{
  if ((p) == null) Lang.ASSERT_NOT_NULL(p, "properties");
  if (StringUtil.isNullOrTrimmedEmpty(key)) Lang.ASSERT_NOT_NULL_NOR_TRIMMED_EMPTY(key, "key");
...
public static Properties loadProperties(Class clazz) throws RuntimeException
{
  return loadResourceProperties(clazz, getResourceFilePathName(clazz));
}
