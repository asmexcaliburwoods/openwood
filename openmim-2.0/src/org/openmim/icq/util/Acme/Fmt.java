...
public class Fmt
{
  ...
  public static String fmt(String s, int minWidth, int flags)
  {
    ...
    throw new InternalError("org.openmim.icq.util.Acme.Fmt: number flag on a non-number");
    if (zeroFill && leftJustify)
    throw new InternalError("org.openmim.icq.util.Acme.Fmt: zero-fill left-justify is silly");
    if (hexadecimal && octal)
    throw new InternalError("org.openmim.icq.util.Acme.Fmt: can't do both hex and octal");
    if (len >= minWidth)
    return s;
