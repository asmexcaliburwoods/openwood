 ...
 * Aim_tlvlist_t constructor comment.
 */
public Aim_tlvlist_t(Session ses, byte[] data)
 ...
 * Aim_tlvlist_t constructor comment.
 */
public Aim_tlvlist_t(Session ses, final byte[] data, final int ofs, final int len)
throws MessagingNetworkException
{
  if ((data) == null) Lang.ASSERT_NOT_NULL(data, "data");
  int pos = ofs;
  while (pos < ofs + len)
  {
    ...
    if (pos < ofs + len)
    {
      ...
      //we d ump any invalid TLVs of that sort.  Hopefully
      ...
      else
      {
        ...
        pos += 2;
        if (length < 0)
          MLang.EXPECT_NON_NEGATIVE(
            length, "length of tlv (of type " + type + " decimal)",
...
/**
  Grab the Nth TLV of type type in the TLV list list.
*/
public long getNthTlvOfTypeAs32Bit(int n, int type)
{
  Aim_tlv_t tlv = getNthTlvOfType(n, type);
  if (tlv == null || tlv.value == null)
    return 0;
  return Session.aimutil_get32(tlv.value, 0);
}
/**
 * Grab the Nth TLV of type type in the TLV list list.
 */
