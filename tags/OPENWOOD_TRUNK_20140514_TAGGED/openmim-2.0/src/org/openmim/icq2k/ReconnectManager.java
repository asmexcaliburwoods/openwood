...
public class ReconnectManager
{
  ...
  private final Scheduler sche = new Scheduler(
    ICQ2KMessagingNetwork.REQPARAM_RECONNECTOR_THREADCOUNT_MAXIMUM,
    ICQ2KMessagingNetwork.REQPARAM_RECONNECTOR_THREADCOUNT_OPTIMUM);
  ...
  private synchronized boolean isInitialised() { return initialised; }
  ...
  private synchronized void runNow(Task t) { checkInitialised(); sche.runNow(t); }
  ...
  */
  ...
  {
    if (approxSeconds > 0) ev.reconnectManagerState = 4;
    else
    {
      ...
      if (ev.isRegisteredReloginsMaximumReached()) //relogins are too frequent
      {
        if (Defines.DEBUG && CAT.isDebugEnabled()) 
          CAT.debug("relogins for "+ev+" are too frequent, sleeping");
        scheduleEvent(ev, 1000L * (
          60 * ICQ2KMessagingNetworkReconnecting.REQPARAM_NETWORK_CONDITIONS_SWING__SLEEP_TIME_WHEN_MAXIMUM_REACHED_MINUTES
          +
    ...
    if (Defines.DEBUG && CAT.isDebugEnabled()) 
      CAT.debug(ev.getLoginId()+": r_sched_event: ev.r_state: "+r_state2string(ev.reconnectManagerState));
    ...
    switch (ev.reconnectManagerState)
    {
      ...
      case 4:  scheduleEvent(ev, (long) (1000*(approxSeconds * (0.7 + Math.random() * 0.3))));  break;
      default: Lang.ASSERT_FALSE("invalid state: "+r_state2string(ev.reconnectManagerState));
    }
  }
  ...
  private synchronized void scheduleEvent(final SessionReconnecting ev, long afterMillis)
  {
    ...
    runAt(
      ...
      ev.reconnectTask = new MessagingTask("reconnect of "+ev, ev)
      {
        public void run() throws Exception
        {
          if (Defines.DEBUG && CAT.isDebugEnabled()) 
            CAT.debug(ev.getLoginId() + ": task launched, attempting reconnect, session r_state="+
              r_state2string(ev.reconnectManagerState));
          ev.reconnect();
        }
      });
  }
